...
import { ShoppingCart, Wallet, Receipt, TrendingUp, AlertTriangle } from "lucide-react";
...
function AgentPortal() {
  ...
  // 1. Fetch agent record associated with logged-in user
  ...
  // 2. Fetch agent's commission statistics and sales history
  const { data: stats, isLoading: loadingStats, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: sales }, { data: commissions }] = await Promise.all([
        supabase
          .from("sales")
          .select("id, invoice_number, sale_date, gross_amount, net_amount, agent_commission_amount, status")
      ...
      const chartData = Array.from({ length: 7 }, (_, i) => {
        ...
        const amount = salesList
          ...
          .reduce((sum, s) => sum + Number(s.agent_commission_amount), 0);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("agent-portal-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "sales", filter: `agent_id=eq.${agent.id}` }, () => refetch())
      .on("postgres_changes", { event: "*", schema: "public", table: "commissions", filter: `agent_id=eq.${agent.id}` }, () => refetch())
      .subscribe();
  ...
  if (loadingAgent || (agent && loadingStats)) {
    return <div className="p-8 text-center text-muted-foreground">Loading agent workspace...</div>;
  }
      ...
      <div className="p-8 max-w-2xl mx-auto mt-12">
        <Card className="p-8 border-destructive/30 bg-destructive/5 text-center">
          ...
          <p className="text-muted-foreground text-sm max-w-md mx-auto">
            Your login user account is not linked to any Travel Agent record.
  ...
  const cards = [
    { label: "Today's Net Sales", value: formatMoney(stats?.todaySales ?? 0), icon: ShoppingCart },
    { label: "This Month Net Sales", value: formatMoney(stats?.monthSales ?? 0), icon: TrendingUp },
    { label: "Pending Commissions Payout", value: formatMoney(stats?.pendingPayout ?? 0), icon: Wallet },
    { label: "Total Commissions Paid", value: formatMoney(stats?.totalEarned ?? 0), icon: Receipt },
  ];
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <PageHeader
          ...
          description={`Travel Agent Portal • Code: ${agent.code} • Commission: ${agent.default_commission_rate}%`}
        />
        <div className="text-xs text-muted-foreground text-right border-l pl-4 border-border hidden md:block">
          <div>Contact: {agent.contact_person || "—"}</div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="flex justify-between py-2 border-b">
              <span className="text-muted-foreground">Agent ID / Code</span>
              <span className="font-medium">{agent.code}</span>
            </div>
            <div className="flex justify-between py-2 border-b">
              <span className="text-muted-foreground">Commission Rate</span>
              <span className="font-medium">{agent.default_commission_rate}%</span>
            </div>
            <div className="flex justify-between py-2 border-b">
      ...
      <Card className="p-6">
        <div className="font-semibold text-sm mb-4">Sales &amp; Commission History</div>
              ...
              {stats?.sales.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground py-8">
                    No sales recorded for your account yet.
                  </TableCell>
                </TableRow>
              ) : (
                stats?.sales.map(s => {
